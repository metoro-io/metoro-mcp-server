...
func main() {
	...
	metoroGetLogs := mcp.NewTool("get_logs",
		mcp.WithDescription("Get logs from all/any services/hosts/pods running in your Kubernetes cluster, monitored by Metoro"),
		...
		mcp.WithString("filters",
			mcp.Description("The filters to apply to the logs. It is a stringified map[string]string[], e.g., '{\"service_name\": [\"/k8s/namespaceX/serviceX\"]}' should return logs for serviceX in namespaceX")),
		mcp.WithString("excludeFilters",
			mcp.Description("The filters that should be excluded from the logs. It is a stringified map[string]string[] e.g., '{\"service_name\": [\"/k8s/namespaceX/serviceX\"]}' should return all logs except for serviceX in namespaceX")),
