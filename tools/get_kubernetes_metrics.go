...
type GetKubernetesMetricsHandlerArgs struct {
	...
	Aggregation model.Aggregation `json:"aggregation" jsonschema:"required,description=The aggregation to apply to the metrics. Possible values are: sum, avg, min, max, count"`
...
type GetKubernetesMetricsRequest struct {
...
func GetKubernetesMetricsHandler(ctx context.Context, arguments GetKubernetesMetricsHandlerArgs) (*mcpgolang.ToolResponse, error) {
	startTime, endTime, err := utils.CalculateTimeRange(arguments.TimeConfig)
	if err != nil {
		return nil, fmt.Errorf("error calculating time range: %v", err)
	}

	request := GetKubernetesMetricsRequest{
		StartTime:      startTime,
		EndTime:        endTime,
	...
	resp, err := utils.MakeMetoroAPIRequest("POST", "kubernetesMetrics", bytes.NewBuffer(jsonBody), utils.GetAPIRequirementsFromRequest(ctx))
	if err != nil {
