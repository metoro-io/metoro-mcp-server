...
type GetLogsHandlerArgs struct {
	...
	Filters        map[string][]string `json:"filters" jsonschema:"description=Filters to apply to the logs. Only the logs that match these filters will be returned. Get the possible filter keys from the get_attribute_keys tool and possible values of a filter key from the get_attribute_values tool. e.g. {service.name: [/k8s/namespaceX/serviceX]} should return logs for serviceX in namespaceX"`
	ExcludeFilters map[string][]string `json:"excludeFilters" jsonschema:"description=Filters to exclude the logs. Logs matching the exclude filters will not be returned. Get the possible exclude filter keys from the get_attribute_keys tool and possible exclude filter values from the get_attribute_values tool (for a key). e.g. {service.name: [/k8s/namespaceX/serviceX]} should exclude logs for serviceX in namespaceX"`
