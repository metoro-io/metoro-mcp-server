...
type SingleMetricRequest struct {
	...
	Filters           map[string][]string    `json:"filters" jsonschema:"description=Filters to apply to the timeseries. Only the timeseries that match these filters will be returned. Get the possible filter keys and values from the get_attribute_keys and get_attribute_values tools. e.g. {service_name: [/k8s/namespaceX/serviceX]} should return timeseries for serviceX in namespaceX. This is just and example. Do not guess the attribute keys and values."`
