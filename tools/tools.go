...
`,
	...
	},
	{
		...
		Description: `Get one or more timeseries data for a metric or traces or logs or kubernetes resources. This tool is useful for understanding how the underlying type of data (specific/metric/trace/kubernetes resources/logs) change over time. You can also apply formulas to combine timeseries to calculate rates or ratios or differences etc. Use the optional title argument to briefly explain what is being queried.
                      ...
                      Metrics of type counter are cumulative metrics but Metoro already shows the count for a specific bucket. For example: container_resources_cpu_usage_seconds_total will return t: 10, t+1: 12. This means that 10 seconds cpu were used in the bucket for t and 12 seconds of cpu were used in the bucket for t+1. Do not calculate a rate or monotonic difference yourself from those bucket values. If the user wants CPU cores, apply the perSecond function to convert CPU-seconds per bucket into average core usage.
`,
